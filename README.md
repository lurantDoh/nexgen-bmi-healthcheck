...
```text
The us-east-1 Function URL is generated by Terraform after deployment.
```

```bash
# Replace FUNCTION_URL with the Terraform output after deployment.
curl -sS "${FUNCTION_URL}/health"
curl -sS "${FUNCTION_URL}/bmi?height_cm=175&weight_kg=70"
```
